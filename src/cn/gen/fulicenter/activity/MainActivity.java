...
import cn.gen.fulicenter.fragment.ChatAllHistoryFragment;
import cn.gen.fulicenter.fragment.ContactlistFragment;
import cn.gen.fulicenter.fragment.SettingsFragment;
...
public class MainActivity extends BaseActivity implements EMEventListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(cn.gen.fulicenter.R.layout.activity_main);
		initView();
		...
		// 添加显示第一个fragment
		getSupportFragmentManager().beginTransaction().add(cn.gen.fulicenter.R.id.fragment_container, chatHistoryFragment)
				.add(cn.gen.fulicenter.R.id.fragment_container, contactListFragment).hide(contactListFragment).show(chatHistoryFragment)
				.commit();
	...
	static void asyncFetchContactsFromServer(){
	    HXSDKHelper.getInstance().asyncFetchContactsFromServer(new EMValueCallBack<List<String>>(){
            ...
            public void onSuccess(List<String> usernames) {
                ...
                String strChat = context.getString(cn.gen.fulicenter.R.string.Application_and_notify);
                newFriends.setNick(strChat);
        
                userlist.put(Constant.NEW_FRIENDS_USERNAME, newFriends);
                // 添加"群聊"
                EMUser groupEMUser = new EMUser();
                String strGroup = context.getString(cn.gen.fulicenter.R.string.group_chat);
...
//        		userlist.put(Constant.CHAT_ROBOT, robotEMUser);
	...
	private void initView() {
		unreadLabel = (TextView) findViewById(cn.gen.fulicenter.R.id.unread_msg_number);
		unreadAddressLable = (TextView) findViewById(cn.gen.fulicenter.R.id.unread_address_number);
		mTabs = new Button[3];
		mTabs[0] = (Button) findViewById(cn.gen.fulicenter.R.id.btn_conversation);
		mTabs[1] = (Button) findViewById(cn.gen.fulicenter.R.id.btn_address_list);
		mTabs[2] = (Button) findViewById(cn.gen.fulicenter.R.id.btn_setting);
		// 把第一个tab设为选中状态
	...
	public void onTabClicked(View view) {
		switch (view.getId()) {
		case cn.gen.fulicenter.R.id.btn_conversation:
			index = 0;
			break;
		case cn.gen.fulicenter.R.id.btn_address_list:
			index = 1;
			break;
		case cn.gen.fulicenter.R.id.btn_setting:
			index = 2;
			break;
		}
		if (currentTabIndex != index) {
			...
			if (!fragments[index].isAdded()) {
				trx.add(cn.gen.fulicenter.R.id.fragment_container, fragments[index]);
			}
...
//					String path = null;
		...
		public void onContactDeleted(final List<String> usernameList) {
			...
			Map<String, EMUser> localUsers = ((DemoHXSDKHelper)HXSDKHelper.getInstance()).getContactList();
			HashMap<String, Contact> userList = SuperWeChatApplication.getInstance().getUserList();
			ArrayList<String> toDeleUserNames = new ArrayList<String>();
			for (String username : usernameList) {
				localUsers.remove(username);
				EMUserDao.deleteContact(username);
				inviteMessgeDao.deleteMessage(username);
				if (userList.containsKey(username)) {
					toDeleUserNames.add(username);
				}
			}
			if(toDeleUserNames.size()>0){
				for (String name:toDeleUserNames){
					try {
						String path = new ApiParams()
                                .with(I.Contact.USER_NAME,SuperWeChatApplication.getInstance().getUserName())
                                .with(I.Contact.CU_NAME,name)
			...
			runOnUiThread(new Runnable() {
				public void run() {
					// 如果正在与此用户的聊天页面
					String st10 = getResources().getString(cn.gen.fulicenter.R.string.have_you_removed);
	...
	public class MyConnectionListener implements EMConnectionListener {
		...
		public void onDisconnected(final int error) {
			final String st1 = getResources().getString(cn.gen.fulicenter.R.string.can_not_connect_chat_server_connection);
			final String st2 = getResources().getString(cn.gen.fulicenter.R.string.the_current_network);
			runOnUiThread(new Runnable() {
	...
	public class MyGroupChangeListener implements EMGroupChangeListener {
		...
		public void onInvitationReceived(String groupId, String groupName, String inviter, String reason) {
			...
			// 被邀请
			String st3 = getResources().getString(cn.gen.fulicenter.R.string.Invite_you_to_join_a_group_chat);
		...
		public void onApplicationAccept(String groupId, String groupName, String accepter) {
			...
			String st4 = getResources().getString(cn.gen.fulicenter.R.string.Agreed_to_your_group_chat_application);
			// 加群申请被同意
			EMMessage msg = EMMessage.createReceiveMessage(Type.TXT);
			msg.setChatType(ChatType.GroupChat);
	...
	private void showConflictDialog() {
		...
		String st = getResources().getString(cn.gen.fulicenter.R.string.Logoff_notification);
		if (!MainActivity.this.isFinishing()) {
			...
			try {
				...
				conflictBuilder.setMessage(cn.gen.fulicenter.R.string.connect_conflict);
				conflictBuilder.setPositiveButton(cn.gen.fulicenter.R.string.ok, new DialogInterface.OnClickListener() {
	...
	private void showAccountRemovedDialog() {
		...
		DemoHXSDKHelper.getInstance().logout(true,null);
		String st5 = getResources().getString(cn.gen.fulicenter.R.string.Remove_the_notification);
		if (!MainActivity.this.isFinishing()) {
			// clear up global variables
			try {
				...
				accountRemovedBuilder.setMessage(cn.gen.fulicenter.R.string.em_user_remove);
				accountRemovedBuilder.setPositiveButton(cn.gen.fulicenter.R.string.ok, new DialogInterface.OnClickListener() {
